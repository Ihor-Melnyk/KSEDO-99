function onBeforeCardSave() {
  ...
  setFormattingFirsNameLastNameInAttr(
    "ApprovalNameI",
    "ApprovalNameF",
    "ApprovalNameF2"
  );
...
function formattingFirsNameLastName(firstName = "", lastName = "") {
  return `${firstName}${lastName ? ` ${lastName.toUpperCase()}` : ""}`;
}

function setFormattingFirsNameLastNameInAttr(
  firstNameCode,
  lastNameCode,
  attrCode
) {
  ...
  EdocsApi.setAttributeValue({
    code: attrCode,
    value: formattingFirsNameLastName(
      EdocsApi.getAttributeValue(firstNameCode).value,
      EdocsApi.getAttributeValue(lastNameCode).value
    ),
    text: null,
  });
}
...
function setRegDateText() {
  if (
    ...
    !EdocsApi.getAttributeValue("RegDateText").value
  ) {
    setDateSTR("RegDate", "RegDateText");
  }
}
